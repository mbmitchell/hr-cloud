  ...
  try {
    ...
    if (!canActorManageEmployeeChangeRequests(actor)) {
      return NextResponse.json(
        { error: "You do not have permission to cancel employee change requests." },
    ...
    if (change.status === "APPLIED" || change.status === "CANCELLED") {
      return NextResponse.json(
        { error: "Applied or cancelled change requests cannot be cancelled again." },
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await writeAuditLog(tx, {
        userId: actor.id,
        action: "EMPLOYEE_CHANGE_REQUEST_CANCEL",
    ...
    if (sendNotification) {
      try {
        await enqueueJobChangeNotifications({
          eventType: "EMPLOYEE_CHANGE_REQUEST_CANCELLED",
          changeRequestId: changeId,
          actorId: actor.id,
        });
      } catch (error) {
        console.error("Failed to enqueue cancelled job change notifications:", error);
      }
    } else {
      await auditNotificationSuppressed({
        ...
        recipientScope: "REQUEST_CREATOR_AND_HR_ADMIN",
    ...
    return NextResponse.json(
      { error: "Failed to cancel employee change request." },
