...
import {
  ...
  getEmployeeMasterPilotTelemetry,
...
export async function GET(request: Request) {
  try {
    const { tenantContext } = await requireRoleWithTenantContext(
      ["SITE_ADMIN", "HR_ADMIN"],
      {
        attemptedAction: "REPORT_EMPLOYEE_MASTER_TELEMETRY_VIEW",
        entityType: "Report",
        entityId: "employee-master-telemetry",
      }
    );
    ...
    const telemetry = await getEmployeeMasterPilotTelemetry({
      filters,
      tenantContext,
    });
    ...
    return NextResponse.json(
      { error: "Failed to load employee master pilot telemetry." },
