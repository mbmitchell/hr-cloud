...
export default function RunRolloverClient() {
  ...
  async function handleRun() {
    ...
    try {
      const response = await fetch("/api/admin/run-rollover", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setMessage(data.error || "Unable to run rollover.");
      } else {
        setResult(data);
        setMessage("Year-end rollover completed.");
      }
    } catch {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Run Year-End PTO Rollover</h2>
        <p className="text-sm text-slate-600 mt-1">
          Applies the rollover cap and creates forfeiture entries for excess PTO.
        </p>
      </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-100 text-left">
                <tr>
                  ...
                  <th className="p-3">Prior Balance</th>
                  <th className="p-3">Forfeited</th>
